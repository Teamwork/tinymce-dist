...
var textpattern = (function () {
  ...
  var $_6pvzb1q4jd24rlei = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_3gx4tqcjd24rleu.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_3gx4tqcjd24rleu.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_3gx4tqcjd24rleu.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_17709pq9jd24rleq.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_17709pq9jd24rleq.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_17709pq9jd24rleq.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_1m9ih6q8jd24rlen.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_1m9ih6q8jd24rlen.checkKeyCode(keyCodes, e)) {
        $_1m9ih6q8jd24rlen.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_1m9ih6q8jd24rlen.checkCharCode(charCodes, e)) {
  ...
  var $_cuycrmq5jd24rlek = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_6pvzb1q4jd24rlei.getPatterns(editor.settings));
    $_cuycrmq5jd24rlek.setup(editor, patternsState);
    return $_bno312q3jd24rleh.get(patternsState);
  });
  function Plugin () {
  }
